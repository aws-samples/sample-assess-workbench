    ...
    def test_trigger_without_document_returns_400(self, api_endpoint, http):
        ...
        try:
            resp = http.post(
                ...
                payload={"name": "E2E no upload", "files": [{"filename": "not-uploaded.txt"}]},
            )
...
class TestReviewEvents:
    ...
    def test_events_endpoint(self, api_endpoint, http):
        ...
        url = f"{api_endpoint}/projects/{self.project_id}/reviews/{self.review_id}/events"
        ...
        assert "review_complete" in event_types, f"Missing review_complete. Got: {event_types}"
    ...
    def test_resolve_latest_events(self, api_endpoint, http):
        ...
        url = f"{api_endpoint}/projects/{self.project_id}/reviews/latest/events"
        resp = http.get(url)
    ...
    def test_feedback_submit_and_read(self, api_endpoint, http):
        ...
        submit_resp = http.post(
            f"{api_endpoint}/projects/{self.project_id}/feedback",
            payload={
                "finding_id": "TEST-001",
        ...
        assert get_resp.status_code == 200, f"Feedback get: {get_resp.status_code} {get_resp.text}"
        body = get_resp.json()
