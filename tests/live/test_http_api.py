...
class TestReferenceData:
    def test_list_agents_returns_array(self, api_endpoint, http):
        ...
        for agent in body["agents"]:
            ...
            assert "icon" in agent and agent["icon"], (
                f"Missing/empty icon: {agent.get('agent_type')}"
            )
            assert "color" in agent and agent["color"], (
                f"Missing/empty color: {agent.get('agent_type')}"
            )
