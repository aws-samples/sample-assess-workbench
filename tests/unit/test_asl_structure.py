...
ASL_PATH = (
    ...
    / "workflow"
    / "statemachine"
    / "review_workflow.asl.json"
)
STATE_MACHINE_TF_PATH = (
    Path(__file__).parent.parent.parent / "terraform" / "modules" / "workflow" / "state_machine.tf"
)
...
class TestReachability:
    ...
    def test_map_iterators_no_orphans(self, asl):
        ...
        for qualified_name, defn in all_states.items():
            ...
            assert not orphaned, f"Orphaned states in {qualified_name} iterator: {orphaned}"
...
class TestAssignResultPath:
    ...
    def test_task_states_with_assign_have_null_result_path(self, asl):
        ...
        assert not violations, (
            f"Task states with Assign but missing ResultPath: null — {violations}"
        )
    ...
    def lambda_contracts(self) -> dict[str, dict]:
        ...
        for placeholder, module_path in _LAMBDA_MODULE_MAP.items():
            ...
            assert file_path.exists(), (
                f"Lambda module not found: {file_path} (placeholder: {placeholder})"
            )
