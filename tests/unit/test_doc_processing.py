...
class TestFormatAnalysisSkipped:
    ...
    def test_skipped_with_description(self):
        result = {
            'type': 'skipped', 'category': 'logo',
            'brief_description': 'Company logo',
            'page_num': 3, 'image_index': 0,
        }
    ...
    def test_skipped_falls_back_to_category(self):
        result = {
            'type': 'skipped', 'category': 'decorative',
            'page_num': 1, 'image_index': 2,
        }
        text = format_analysis_for_injection(result)
...
class TestFormatAnalysisFailedAndTimeout:
    ...
    def test_analysis_failed(self):
        result = {'type': 'analysis_failed', 'page_num': 5, 'image_index': 0,
                  'filename': 'doc.pdf', 'category': 'architecture_diagram'}
        text = format_analysis_for_injection(result)
...
class TestFormatAnalysisAnalyzed:
    ...
    def full_analysis_result(self):
        ...
        return {
            ...
            'analysis': {
                ...
                'connections': [
                    {'from': 'ALB', 'to': 'ECS Cluster', 'label': 'HTTPS', 'direction': 'unidirectional'},
                    {'from': 'ECS Cluster', 'to': 'RDS', 'label': '', 'direction': 'bidirectional'},
                ],
                'groupings': [
                    {'name': 'Production VPC', 'type': 'VPC', 'contains': ['ALB', 'ECS Cluster', 'RDS']},
                ],
    ...
    def test_minimal_analysis_only_required_fields(self):
        ...
        result = {
            'type': 'analyzed', 'page_num': 1, 'image_index': 0,
            'category': 'flowchart',
            'analysis': {},
        }
        text = format_analysis_for_injection(result)
...
class TestInjectImageAnalyses:
    ...
    def test_filename_filtering(self):
        ...
        results = [
            {'type': 'timeout', 'message': '[img from A]', 'filename': 'a.pdf',
             'page_num': 1, 'image_index': 0},
            {'type': 'timeout', 'message': '[img from B]', 'filename': 'b.pdf',
             'page_num': 1, 'image_index': 0},
        ]
    ...
    def test_no_filename_includes_all(self):
        ...
        results = [
            {'type': 'timeout', 'message': '[A]', 'filename': 'a.pdf',
             'page_num': 1, 'image_index': 0},
            {'type': 'timeout', 'message': '[B]', 'filename': 'b.pdf',
             'page_num': 2, 'image_index': 0},
        ]
    ...
    def test_no_matching_filename_returns_unchanged(self):
        results = [
            {'type': 'timeout', 'message': '[x]', 'filename': 'other.pdf',
             'page_num': 1, 'image_index': 0},
        ]
...
# extract_text_and_images_from_pdf
#
# Tests use the real demo PDF from test-docs/ — the same document used
...
class TestExtractTextAndImagesFromPdf:
    ...
    PDF_PATH = Path(__file__).parent.parent.parent / "test-docs" / "mortgage-processing-hld-demo.pdf"
    ...
    def test_image_has_required_fields(self, extracted):
        _, _, images = extracted
        assert len(images) >= 1
        required_keys = {'image_bytes', 'page_num', 'image_index', 'format', 'width', 'height', 'raw_format'}
