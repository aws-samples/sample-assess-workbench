...
class TestBuildOrgContext:
    ...
    def test_coach_feedback_includes_prior_findings(self):
        ...
        result = build_org_context(
            "Base",
            prior_critique="Needs more depth",
    ...
    def test_coach_feedback_includes_strengths_and_gaps(self):
        result = build_org_context(
            "Base",
            prior_critique="Improve specificity",
    ...
    def test_coach_feedback_includes_critique(self):
        result = build_org_context(
            "Base",
            prior_critique="Add OWASP mappings",
            iteration=2,
        )
    ...
    def test_coach_mode_appends_prior_critique(self):
        result = build_judge_prompt(
            ...
            mode="coach",
            iteration=2,
            prior_critique="Missing encryption",
        )
    ...
    def test_evaluate_mode_no_prior_critique(self):
        result = build_judge_prompt(
            ...
            mode="evaluate",
            iteration=0,
            prior_critique="Should not appear",
        )
...
class TestJudgeTool:
    """JUDGE_TOOL is the Bedrock tool schema used by invoke_judge.

    These tests ensure the schema stays importable from core.coach_logic and
    structurally valid.
    """
    ...
    def test_scoring_criteria_present(self):
        criteria = JUDGE_TOOL["toolSpec"]["inputSchema"]["json"]["properties"][
            "scores_by_criterion"
        ]["properties"]
