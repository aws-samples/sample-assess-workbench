...
| Attribute | Purpose |
|-----------|---------|
| `PK` (hash) | Entity-type prefix + identifier, e.g. `PROJECT#<id>`, `CONTEXT#<id>`, `BENCHMARK#<id>`, or the literal string `AGENT_REGISTRY` |
...
erDiagram
    ...
    BENCHMARK {
        string partition_key "BENCHMARK prefix"
        string sort_key "METADATA"
        string gsi1_partition "BENCHMARK"
        string gsi1_sort "created_at"
        string name
        string status
        string agent_type
    ...
    RUN {
        string partition_key "BENCHMARK parent"
        string sort_key "RUN config_id"
        string status
        json findings
        json quality_scores
    }
...
| List benchmarks | `Query GSI1 PK=BENCHMARK` |
...
The `GSI1` index is deliberately sparse. Only the `METADATA` item for each entity (project, context, benchmark) sets `GSI1PK`, so the index contains exactly one row per entity. Children (`REVIEW#`, `PLAN#`, `CHAT#`, `EVENT#`, `FEEDBACK#`) omit `GSI1PK` and don't appear in the index.
...
- **Standards Knowledge Base** (Bedrock KB, S3 Vectors backend) holds the compliance standards corpus (OWASP Top 10, CWE, ISO 31000, GDPR, DORA). Populated once at deploy time from `standards/`; re-synced on demand with `--sync-only`.
