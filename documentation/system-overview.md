...
A single DynamoDB table holds projects, reviews, plans, chat history, progress events, feedback, contexts, the agent registry, and benchmark definitions using a `PK`/`SK` single-table design with two GSIs. A separate connections table tracks live WebSockets with TTL cleanup. User-uploaded documents and workflow artifacts live in one S3 bucket; the SPA is served from a second S3 bucket behind CloudFront.
...
For the full pipeline structure and SSM handoff, see [deployment-architecture.md](deployment-architecture.md). For the operator runbook, see [../DEPLOYMENT.md](../DEPLOYMENT.md).
...
- [data-model.md](data-model.md) — DynamoDB + S3 layout
- [deployment-architecture.md](deployment-architecture.md) — How deploy pipelines fit together
