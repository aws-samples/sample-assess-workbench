...
graph TB
    ...
    subgraph Compute["Compute"]
        ...
        BenchmarkLambda[Benchmark Lambda<br/>comparative agent runs]
    end
    ...
    APILambda -->|invoke on-demand| BenchmarkLambda
    ...
    BenchmarkLambda --> AgentCore
    BenchmarkLambda --> Converse
    BenchmarkLambda --> DDB
    ...
    class APILambda,WSLambdas,WorkflowLambdas,BenchmarkLambda compute
    class SFN orch
...
| Benchmark | 1 Lambda | Ad-hoc tool for running parallel agent comparisons. Invoked on demand by the REST API, not by Step Functions. |
...
| **Bedrock Knowledge Base — Standards** | RAG store for compliance standards (OWASP, CWE, ISO 31000, GDPR, DORA). Populated once at deploy time from `standards/` corpus, re-synced on demand with `--sync-only`. |
