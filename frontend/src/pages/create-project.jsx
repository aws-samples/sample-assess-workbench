...
// Mirror the backend's extraction support (api/workflow/load_document.py:
// PDF via PyMuPDF + the UTF-8 text family). Keep in sync — a format not listed
// here is not extracted and would yield an empty review, so we block it at upload.
const ACCEPTED_EXTENSIONS = ['pdf', 'txt', 'md', 'markdown', 'rst', 'csv', 'json', 'yaml', 'yml'];
const ACCEPTED_TYPES = ACCEPTED_EXTENSIONS.map((e) => `.${e}`).join(',');
const MAX_FILES = 10;
...
export function CreateProjectPage() {
  ...
  function addFiles(newFiles) {
    // The input's `accept` attribute only filters the file-picker dialog;
    // drag-and-drop bypasses it. Validate here so both paths reject formats
    // the backend can't extract (which would otherwise yield an empty review).
    const accepted = [];
    const rejected = [];
    for (const f of Array.from(newFiles)) {
      const ext = f.name.split('.').pop().toLowerCase();
      if (ACCEPTED_EXTENSIONS.includes(ext)) accepted.push(f);
      else rejected.push(f.name);
    }
    if (rejected.length) {
      setStatus({ type: 'error', message: t('create.unsupportedFiles', { files: rejected.join(', ') }) });
    } else if (accepted.length) {
      // A clean batch clears any lingering "unsupported file" error.
      setStatus(null);
    }
    if (accepted.length) {
