          ...
          <div class="findings-panel">
              ...
              <div class="findings-scroll custom-scroll">
                ...
                {findings.length === 0 ? (
                  ...
                  findings.map((f, i) => <FindingCard key={f.id || i} finding={f} fieldDefinitions={(agents[currentTab]?.finding_schema?.fields) || {}} forceOpen={expandAll} feedbackValue={feedbackMap[`${currentTab}:${f.id}`] || null} feedbackError={feedbackErrors[`${currentTab}:${f.id}`] || null} onFeedback={(fid, val) => handleFeedback(currentTab, fid, val)} />)
                )}
              </div>
          </div>
