...
**AgentCore Risk Assessor API** v2.0.0
...
Creates a new benchmark that compares multiple model/prompt configurations against the same document. Use POST /benchmarks/{id}/run to execute.

**Request body:**

- `name` (string) *(required)* — Human-readable benchmark name
- `document_project_id` (string) *(required)* — Source project ID for the document to benchmark against
- `agent_type` (string) *(required)* — Agent type to benchmark
- `configurations` (array) *(required)*
  *Array items:*
    - `config_id` (string) — Unique config identifier (auto-generated if omitted)
    - `label` (string) *(required)* — Human-readable label
    - `model_id` (string) *(required)* — Bedrock model ID
    - `depth` (string)
    - `prompt_version` (string)

**Response (201):** Benchmark created
...
Returns all benchmark definitions, newest first.

**Response (200):** List of benchmarks
...
**Get benchmark with run results**

Returns the benchmark definition and all run results.

**Parameters:**

- `benchmark_id` (path) *(required)* — Unique benchmark identifier

**Response (200):** Benchmark with runs

---

### `DELETE /benchmarks/{benchmark_id}`
**Delete benchmark and results**

Deletes the benchmark definition and all associated run results.

**Parameters:**

- `benchmark_id` (path) *(required)* — Unique benchmark identifier

**Response (200):** Benchmark deleted
...
### `POST /benchmarks/{benchmark_id}/run`
**Execute benchmark**

Triggers execution of all configurations in parallel. Each configuration invokes the review agent and judge evaluator, storing results as run records. Returns immediately with 202 — execution is async...

**Parameters:**

- `benchmark_id` (path) *(required)* — Unique benchmark identifier

**Response (202):** Benchmark execution started
...
Returns foundation models available for benchmarking. Filtered to text-output, on-demand, active models.

**Response (200):** List of available models
