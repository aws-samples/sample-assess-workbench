...
def _resolve_table_name(table_name: Optional[str] = None) -> str:
    ...
    """
    resolved = (
        table_name or os.environ.get("PROJECTS_TABLE") or os.environ.get("DYNAMODB_TABLE_NAME", "")
    )
    if not resolved:
        raise RuntimeError(
...
def query_agent_registry(
    ...
    *,
    ...
    try:
        ...
        raise RuntimeError(f"Failed to query agent registry from DynamoDB: {e}") from e
...
def get_agent_registry_entry(table, agent_type: str) -> Optional[Dict[str, Any]]:
    ...
    """
    try:
        response = table.get_item(Key={"PK": "AGENT_REGISTRY", "SK": f"AGENT#{agent_type}"})
    except Exception as e:
        raise RuntimeError(f'Failed to read agent registry entry "{agent_type}": {e}') from e
...
def load_agent_arns(
    ...
    Raises:
        RuntimeError: If the table name cannot be resolved, ``PROJECT_NAME`` or
            ``ENVIRONMENT`` is unset, or the registry/SSM cannot be queried.
    """
    ...
    if not project_name or not environment:
        raise RuntimeError(
            "Agent registry unavailable: PROJECT_NAME and ENVIRONMENT must be set "
            "(needed for the SSM agent-ARN prefix /{project}/{environment}/agent/)."
        )
    ...
    try:
        ...
        raise RuntimeError(f"Failed to read SSM parameters under {ssm_prefix}: {e}") from e
