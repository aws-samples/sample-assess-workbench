...
# ── Judge tool schema (shared by invoke_judge and run_benchmark) ──
...
JUDGE_TOOL = {
    'toolSpec': {
        ...
        'inputSchema': {
            'json': {
                'type': 'object',
                'required': ['quality_met', 'overall_score', 'scores_by_criterion', 'strengths', 'gaps'],
                'properties': {
                    'quality_met': {
                        'type': 'boolean',
...
def build_org_context(base_context, depth='standard', focus_areas=None,
                      prompt_addendum='', prior_critique='', prior_findings=None,
                      prior_strengths=None, prior_gaps=None, iteration=0):
...
def build_judge_prompt(agent_type, quality_threshold, coach_guidance,
                       document_preview, focus_areas, findings,
                       mode='evaluate', iteration=0, prior_critique=''):
        ...
        for agent in group.get('agents', []):
            ...
            if isinstance(judge_defaults, dict) and judge_defaults:
                ...
                if 'quality_threshold' in judge_defaults:
                    try:
                        judge['quality_threshold'] = round(float(judge_defaults['quality_threshold']), 2)
