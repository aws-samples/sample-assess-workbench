...
if not _CONNECTIONS_TABLE:
    logger.info("CONNECTIONS_TABLE not configured — WebSocket broadcast disabled, falling back to single connection_id")
if not _EVENTS_TABLE:
    logger.info("DYNAMODB_TABLE_NAME not configured — event persistence disabled, progress events will not be replayed on page refresh")
