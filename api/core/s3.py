...
def read_json(bucket: str, key: str) -> Any:
    ...
    try:
        ...
        raise RuntimeError(
            f"Failed to read JSON from s3://{bucket}/{key}: {e}"
        ) from e
...
def write_json(bucket: str, key: str, data: Any) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            f"Failed to write JSON to s3://{bucket}/{key}: {e}"
        ) from e
...
def read_text(bucket: str, key: str) -> str:
    ...
    try:
        ...
        raise RuntimeError(
            f"Failed to read text from s3://{bucket}/{key}: {e}"
        ) from e
...
def head_object(bucket: str, key: str) -> dict:
    ...
    try:
        ...
        raise RuntimeError(
            f"Failed to HEAD s3://{bucket}/{key}: {e}"
        ) from e
...
def read_bytes(bucket: str, key: str) -> bytes:
    ...
    try:
        ...
        raise RuntimeError(
            f"Failed to read bytes from s3://{bucket}/{key}: {e}"
        ) from e
