...
def format_analysis_for_injection(result: dict) -> str:
    ...
    if result['type'] == 'skipped':
        return (f"[Image skipped: {result.get('brief_description', result['category'])} "
    ...
    lines.append(f"--- [Diagram: {diagram_type} — Page {result['page_num']}, Image {result['image_index'] + 1}] ---")
...
def inject_image_analyses(
    text_content: str, analysis_results: list[dict], filename: str | None = None,
) -> str:
    ...
    file_results = [r for r in analysis_results if not filename or r.get('filename', '') == filename]
    if not file_results:
        return text_content
