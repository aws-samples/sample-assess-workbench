...
connections_table = dynamodb.Table(_connections_table_name) if _connections_table_name else None
...
def send_to_connection(connection_id, endpoint_url, data):
    ...
    try:
        ...
        client.post_to_connection(
            ConnectionId=connection_id, Data=json.dumps(data, default=str).encode("utf-8")
        )
        return True
...
def build_agent_payload(
    ...
    return {
        ...
        "chat_history": load_chat_history(projects_table, project_id, user_sub, agent, session_id),
...
def handle_delete_session(body, connection_id, endpoint_url, user_sub: str = ""):
    ...
    items_deleted = delete_chat_session(projects_table, project_id, user_sub, agent, session_id)
    send_to_connection(
        connection_id,
        endpoint_url,
        {
...
def handle_load_history(body, connection_id, endpoint_url, user_sub: str = ""):
    ...
    messages = load_chat_messages(projects_table, project_id, user_sub, agent, session_id)
    send_to_connection(
        connection_id,
        endpoint_url,
        {
            ...
            if line:
                ...
                if decoded.startswith("data: "):
                    ...
                    if isinstance(data, dict) and "type" in data:
                        data["agent"] = agent
                        send_to_connection(connection_id, endpoint_url, data)
                        continue
                    ...
                    logger.info(
                        f"SSE #{chunk_count}: {len(chunk_text)} chars, elapsed {elapsed:.0f}ms"
                    )
