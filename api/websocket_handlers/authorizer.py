...
Uses PyJWT for full cryptographic signature verification against
...
def _get_signing_key(token):
    ...
    # Read the (unverified) header to get the kid; signature is verified later.
...
def validate_token(token):
    ...
    """
    signing_key_jwk = _get_signing_key(token)

    # Cognito JWKS keys are RSA; convert the matched JWK to a public key object.
    public_key = RSAAlgorithm.from_jwk(json.dumps(signing_key_jwk))

    # Verify signature and decode claims. PyJWT checks exp/nbf/iat/aud/iss.
    payload = jwt.decode(
        token,
        public_key,
        algorithms=["RS256"],
    ...
    try:
        ...
        if ORIGIN_VERIFY_SECRET:
            ...
            if not hmac.compare_digest(header_value, ORIGIN_VERIFY_SECRET):
                logger.warning(
                    "Origin verification failed — request did not come through CloudFront"
                )
