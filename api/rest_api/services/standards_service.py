...
class StandardsService:
    ...
    def upload_standard(
        ...
        self.s3.put_document(
            sidecar_key, json.dumps(sidecar, indent=2), content_type="application/json"
        )
    ...
    def update_metadata(
        ...
        logger.info(
            "Updated metadata sidecar for standard '%s' (content: %s)", standard_id, content_key
        )
    ...
    def _find_content_key(self, standard_id: str) -> str:
        ...
        raise ValueError(f"Standard '{standard_id}' not found — no content file under {prefix}")
