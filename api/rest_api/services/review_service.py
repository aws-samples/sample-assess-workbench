...
class ReviewService:
    ...
    def trigger_review(self, project_id: str, user_sub: str = "") -> Optional[Dict[str, Any]]:
        ...
        if not files:
            raise ValueError(
                "No files associated with project. Upload at least one document before triggering review."
            )

        # Check at least the canonical file exists
        if not self.s3.document_exists(s3_key):
            raise ValueError(
                "Document not uploaded yet. Please upload document to S3 before triggering review."
            )
        ...
        connection_id = (
            get_most_recent_connection(self.connections_table, user_sub) if user_sub else ""
        )
    ...
    def approve_review_plan(
        self, project_id: str, review_id: str, plan_override: Dict = None
    ) -> Dict[str, Any]:
        ...
        self.dynamodb.update_plan_status(
            project_id, review_id, "approved", approved_plan=approved_plan, modified=modified
        )
    ...
    def reject_review_plan(
        self, project_id: str, review_id: str, reason: str = ""
    ) -> Dict[str, Any]:
    ...
    def abort_review(self, project_id: str, review_id: str, reason: str = "") -> Dict[str, Any]:
        ...
        if project.get("status") != "in_progress":
            raise ValueError(
                f"Project is not in progress (status: {project.get('status', 'unknown')})"
            )
