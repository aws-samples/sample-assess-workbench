...
from core.doc_processing import unsupported_filenames, SUPPORTED_DOCUMENT_EXTENSIONS
...
class ProjectService:
    ...
    def create_project(
        ...
        if not files:
            raise ValueError(
                'At least one file is required. Provide a "files" array with filename entries.'
            )
        ...
        rejected = unsupported_filenames(f["filename"] for f in files)
        if rejected:
            supported = ", ".join(sorted(SUPPORTED_DOCUMENT_EXTENSIONS))
            raise ValueError(
                f"Unsupported file type(s): {', '.join(rejected)}. Supported formats: {supported}."
            )
    ...
    def list_projects(self, status_filter: str = None, limit: int = 50) -> Dict[str, Any]:
        ...
        return {"projects": formatted_projects, "count": len(formatted_projects)}
