...
from rest_api.data_access import DynamoDBDataAccess, S3DataAccess, StepFunctionsDataAccess, BenchmarkDataAccess, GuardrailEventsAccess
from rest_api.services import ProjectService, ReviewService, ContextService, FeedbackService, BenchmarkService, AnalyticsService, AdminService, StandardsService
from rest_api.handlers import ProjectHandlers, ReviewHandlers, ContextHandlers, FeedbackHandlers, BenchmarkHandlers, AnalyticsHandlers, AdminHandlers, StandardsHandlers
...
benchmark_access = BenchmarkDataAccess(dynamodb_access.table)

# Connections table for WebSocket connection lookup
_connections_table_name = os.environ.get('CONNECTIONS_TABLE', '')
_connections_table = boto3.resource('dynamodb').Table(_connections_table_name) if _connections_table_name else None

# Initialize services
project_service = ProjectService(dynamodb_access, s3_access)
review_service = ReviewService(dynamodb_access, s3_access, stepfunctions_access, connections_table=_connections_table)
context_service = ContextService(dynamodb_access, s3_access)
feedback_service = FeedbackService(dynamodb_access)
benchmark_service = BenchmarkService(dynamodb_access, s3_access, benchmark_access)
analytics_service = AnalyticsService(dynamodb_access, benchmark_access)
...
standards_service = StandardsService(standards_s3, kb_id=_standards_kb_id, ds_id=_standards_ds_id) if standards_s3 else None
...
benchmark_handlers = BenchmarkHandlers(benchmark_service)
...
ROUTES: List[Tuple[str, str, Callable]] = [
    ...
    ('GET',    '/models',                                              admin_handlers.list_models),
    ...
    ('POST',   '/projects/{project_id}/reviews/{review_id}/approve',   review_handlers.approve_review_plan),
    ('POST',   '/projects/{project_id}/reviews/{review_id}/reject',    review_handlers.reject_review_plan),
    ...
    ('PUT',    '/contexts/{context_id}/content',                       require_write_access(context_handlers.update_context_content)),
    ...
    # Benchmarks
    ('POST',   '/benchmarks',                                          require_write_access(benchmark_handlers.create_benchmark)),
    ('GET',    '/benchmarks',                                          benchmark_handlers.list_benchmarks),
    ('POST',   '/benchmarks/{benchmark_id}/run',                       require_write_access(benchmark_handlers.run_benchmark)),
    ('GET',    '/benchmarks/{benchmark_id}',                           benchmark_handlers.get_benchmark),
    ('DELETE', '/benchmarks/{benchmark_id}',                           require_write_access(benchmark_handlers.delete_benchmark)),
    ...
    ('GET',    '/admin/standards/sync-status',                         standards_handlers.get_sync_status if standards_handlers else _standards_not_configured),
    ('POST',   '/admin/standards/sync',                                require_write_access(standards_handlers.start_sync) if standards_handlers else _standards_not_configured),
    ('GET',    '/admin/standards',                                     standards_handlers.list_standards if standards_handlers else _standards_not_configured),
    ('POST',   '/admin/standards',                                     require_write_access(standards_handlers.upload_standard) if standards_handlers else _standards_not_configured),
    ('PUT',    '/admin/standards/{standard_id}/metadata',              require_write_access(standards_handlers.update_metadata) if standards_handlers else _standards_not_configured),
    ('DELETE', '/admin/standards/{standard_id}',                       require_write_access(standards_handlers.delete_standard) if standards_handlers else _standards_not_configured),
...
_COMPILED_ROUTES = [
    (method, _compile_route(pattern), handler)
