...
EXPECTED_EVENT = {
    ...
    'optional': ['project_name', 'project_description', 'plan', 'group_results',
                 'connection_id', 'user_sub', 'websocket_endpoint',
                 'image_analysis_metrics', 'document_s3_key', 'planning_duration_ms'],
}
...
def lambda_handler(event: dict, context) -> dict:
    ...
    for agent_type, status_info in agent_statuses.items():
        if status_info['status'] != 'completed':
            send_progress(connection_id, websocket_endpoint, f"agent_{status_info['status']}", {
                'agent': agent_type,
    ...
    if SHARED_MEMORY_ARN and reviews:
        try:
            ...
            send_progress(connection_id, websocket_endpoint, 'memory_write', {
    ...
    send_progress(connection_id, websocket_endpoint, 'aggregation_complete', {
    ...
    if quality_inputs:
        send_progress(connection_id, websocket_endpoint, 'quality_evaluation_started', {
...
def _store_findings_in_memory(
    reviews: dict, project_id: str, project_name: str, project_description: str,
) -> int:
