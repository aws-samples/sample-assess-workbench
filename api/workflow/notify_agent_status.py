...
def _check_group_results(event: dict, context) -> list:
    ...
    failed_agents = [r["agent_type"] for r in group_results if r.get("status") == "failed"]
    ...
    if failed_agents:
        ...
        raise AgentGroupFailedError(f"Aborting review: agents failed: {', '.join(failed_agents)}")
