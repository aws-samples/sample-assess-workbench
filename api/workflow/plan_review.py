...
PLANNER_MODEL_ID = os.environ.get("PLANNER_MODEL_ID")
if not PLANNER_MODEL_ID:
    raise RuntimeError(
        "PLANNER_MODEL_ID environment variable is required but not set. "
...
def lambda_handler(event: dict, context) -> dict:
    ...
    agent_descriptions = "\n".join(
        f"- {name}: {info['description']}" for name, info in available_agents.items()
    )
    ...
    if not plan:
        raise RuntimeError("Planner did not return tool use — unexpected Bedrock response format")
