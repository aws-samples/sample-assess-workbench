...
IMAGE_ANALYSIS_MODEL_ID = os.environ.get("IMAGE_ANALYSIS_MODEL_ID")
if not IMAGE_ANALYSIS_MODEL_ID:
    raise RuntimeError(
        "IMAGE_ANALYSIS_MODEL_ID environment variable is required but not set. "
        "Check the Lambda configuration."
    )

# Supported text file extensions are defined once in core.doc_processing
# (TEXT_DOCUMENT_EXTENSIONS) and shared with the upload API.
        ...
        "inputSchema": {
            "json": {
                ...
                "properties": {
                    ...
                    "relevant": {
                        "type": "boolean",
                        "description": "true if this image contains technical/architectural content worth analyzing",
                    },
                    "confidence": {
                        "type": "number",
        ...
        "inputSchema": {
            "json": {
                ...
                "properties": {
                    ...
                    "summary": {
                        "type": "string",
                        "description": "2-3 sentence summary of what the diagram shows",
                    },
                    "components": {
                        "type": "array",
                        "items": {
                            ...
                            "properties": {
                                ...
                                "type": {
                                    "type": "string",
                                    "description": "e.g. AWS Lambda, database, microservice, load balancer",
                                },
                                "details": {"type": "string"},
                            },
                        },
...
def analyze_images(
    ...
    for idx, img_info in enumerate(images):
        ...
        if elapsed > IMAGE_ANALYSIS_TIMEOUT_S:
            ...
            logger.warning(
                f"Image analysis timeout after {elapsed:.1f}s — {remaining} images not processed"
            )
            results.append(
                {
                    "type": "timeout",
        ...
        results.append(
            {
                ...
                "filename": img_info.get("filename", ""),
                "category": "overflow",
                "brief_description": "Exceeded max image limit",
            }
        )
        metrics["images_skipped"] += 1
    ...
    metrics["total_tokens"] = (
        metrics["triage_tokens"]["input"]
        + metrics["triage_tokens"]["output"]
        + metrics["analysis_tokens"]["input"]
        + metrics["analysis_tokens"]["output"]
    )
...
def _process_file(
    ...
    elif ext in TEXT_DOCUMENT_EXTENSIONS or ext == "":
        try:
...
def _load_project_context(
    ...
    if context_id:
        ...
        if ctx_meta and ctx_meta.get("s3_bucket") and ctx_meta.get("s3_key"):
            organizational_context = read_text(ctx_meta["s3_bucket"], ctx_meta["s3_key"])
            send_progress(
                connection_id,
                websocket_endpoint,
                "context_s3_read",
...
def lambda_handler(event: dict, context) -> dict:
    ...
    # Backstop for the upload-time guard in the create API: a project created
    # before that guard existed (or via any non-API path) can still hold an
    # unextractable file. Fail loudly here rather than reviewing a
    # "[Binary file…]" placeholder and returning an empty review.
    rejected = unsupported_filenames(
        f.get("filename", f["s3_key"].rsplit("/", 1)[-1]) for f in files
    )
    if rejected:
        raise ValueError(
            f"Cannot review unsupported file type(s): {', '.join(rejected)}. "
            "Supported formats: PDF, Markdown, and plain text "
            "(.txt, .md, .csv, .json, .yaml). Re-create the project with a "
            "supported file."
        )
    ...
    for idx, file_info in enumerate(files):
        ...
        if result["skipped"]:
            ...
            combined_sections.append(
                {
                    "filename": filename,
                    "text": f"[Skipped: {filename} — {result['size_mb']}MB exceeds {result['limit_mb']}MB limit]",
                }
            )
