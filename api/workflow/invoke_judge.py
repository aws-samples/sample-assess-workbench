...
EXPECTED_EVENT = {
    'required': ['agent_type'],
    'optional': ['focus_areas', 'quality_threshold', 'mode', 'iteration',
                 'prior_critique', 'prior_score', 'consecutive_drops',
                 'coach_durations', 'coach_guidance',
                 'findings', 'summary', 's3_bucket', 'document_s3_key',
                 'findings_s3_key', 'document_preview',
...
def _load_context_from_s3(
    event: dict, agent_type: str, findings: list, summary: str,
) -> tuple[list, str, str]:
...
def lambda_handler(event: dict, context) -> dict:
    ...
    send_progress(connection_id, websocket_endpoint, 'judge_started', {
        'agent': agent_type,
        'mode': mode,
        'iteration': iteration,
    })
    ...
    early_exit, consecutive_drops = check_early_exit(
        evaluation, overall, mode, iteration,
        float(event.get('prior_score', 0)),
        int(event.get('consecutive_drops', 0)),
    )

    scores = evaluation['scores_by_criterion']
    logger.info(f"Judge {mode} for {agent_type}: scores=({scores['completeness']:.2f}, {scores['specificity']:.2f}, {scores['actionability']:.2f}) "
    ...
    if mode == 'coach':
        send_progress(connection_id, websocket_endpoint, 'judge_evaluated', {
            'agent': agent_type,
            'score': overall,
        ...
        send_progress(connection_id, websocket_endpoint, 'quality_score', {
            'agent': agent_type,
            'overall': overall,
            'scores': scores,
            'quality_met': evaluation['quality_met'],
