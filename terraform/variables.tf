...
variable "deploy_frontend" {
  description = "Deploy the frontend hosting infrastructure (S3 + CloudFront + WAF). False for dev (use Vite dev server locally)."
  type        = bool
  default     = false
}

variable "enable_api_proxy" {
  description = "Route API and WebSocket traffic through CloudFront (unified ingress with WAF). Requires deploy_frontend = true. Set to true in staging/prod tfvars."
  type        = bool
  default     = false
}
...
variable "federated_provider_ready" {
  description = "Set to true after uploading the OIDC client secret to Secrets Manager. Gates creation of the identity provider."
  type        = bool
  default     = false
}
