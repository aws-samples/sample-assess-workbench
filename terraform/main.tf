provider "aws" {
  ...
  # Refuse to operate against an unexpected account: if the live credentials
  # resolve to an account not in this list, the provider fails initialization
  # before any resource is touched. Empty aws_account_id → null → guard disabled
  # (single-account path).
...
resource "null_resource" "build_dependencies_layer" {
  triggers = {
    ...
    # Layer deps are exported from uv.lock at build time (see
    # build_dependencies_layer.sh + pyproject.toml [dependency-groups]
    # lambda_layer), so the lockfile is the dependency trigger. Over-triggers on
    # unrelated dep changes, but a rebuild that produces an identical layer
    # leaves source_code_hash unchanged — harmless.
    lock_hash = filesha256("${path.module}/../uv.lock")
  }
...
module "auth" {
  ...
  federated_client_secret     = var.federated_client_secret
}
...
# Canonical encoding of frontend/public/config.json for the active environment.
# The `frontend:config` task materializes it via
# `terraform output -raw frontend_config_json`; build, dev, and deploy:frontend
# regenerate the file from this single source.
output "frontend_config_json" {
  description = "Rendered frontend/public/config.json content for the active environment."
  value = jsonencode({
