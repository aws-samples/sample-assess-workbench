...
resource "null_resource" "build_load_document" {
  triggers = {
    ...
    # load_document deps are exported from uv.lock at build time (see
    # build_load_document.sh + pyproject.toml [dependency-groups] load_document),
    # so the lockfile is the dependency trigger. Hashing the lock over-triggers
    # on unrelated dep changes, but a rebuild that produces an identical package
    # leaves the Lambda's source_code_hash unchanged — so it's harmless.
    requirements_hash = filesha256("${path.module}/../../../uv.lock")
  }
