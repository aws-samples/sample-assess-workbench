...
resource "aws_cognito_user_pool_client" "frontend" {
  ...
  supported_identity_providers         = var.federated_login_enabled && var.federated_provider_ready ? concat(["COGNITO"], [var.federated_identity_provider]) : ["COGNITO"]
...
resource "aws_cognito_user_group" "admins" {
  name         = "admins"
  user_pool_id = aws_cognito_user_pool.main.id
  description  = "Administrators with access to agent registry management"
}
...
# Read-only viewer group — own projects, read + chat only, no modifications
resource "aws_cognito_user_group" "viewers" {
  name         = "viewers"
  user_pool_id = aws_cognito_user_pool.main.id
  description  = "Read-only viewers with chat access to their own projects"
}
