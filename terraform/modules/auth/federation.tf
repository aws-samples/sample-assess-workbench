...
#   2. apply once to create the secret container,
#   3. upload the OIDC client secret to that secret,
#   4. set federated_provider_ready = true and apply again.

# =============================================================================
# Secrets Manager — OIDC client secret container
# =============================================================================
# The secret value is uploaded out-of-band and is never stored in Terraform
# state. It is only read once federated_provider_ready = true.
...
data "aws_secretsmanager_secret_version" "federated" {
  count     = var.federated_login_enabled && var.federated_provider_ready ? 1 : 0
...
# =============================================================================
# Gated by federated_provider_ready — only created after the client secret has
# been uploaded to Secrets Manager.
resource "aws_cognito_identity_provider" "federated" {
  ...
  provider_details = {
    ...
    client_secret             = data.aws_secretsmanager_secret_version.federated[0].secret_string
