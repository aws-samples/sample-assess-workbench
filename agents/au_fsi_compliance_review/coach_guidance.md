...
- Are remediation timeframes realistic and proportionate to the severity and
  nature of the gap?
- Do recommendations stay within compliance remediation rather than prescribing
  strategic moves the risk agent and the org own? A recommendation to migrate
  platforms, switch vendors, or accelerate adoption of a specific provider is out
  of scope — the finding should recommend the compliance action (document the
  exit plan, escalate the appetite breach, define tolerance levels, test
  transition), and prescribing the strategy instead should be flagged as a
  rubric miss.
