...
def _validate_severity_matrix(schema_config: dict[str, Any]) -> None:
    ...
    for name in input_names:
        ...
        if not enums:
            raise ValueError(
                f"Severity derivation input '{name}' has no enum values defined"
            )
...
def build_finding_model(
    agent_type: str, schema_config: dict[str, Any]
) -> type[BaseModel]:
...
def derive_severity(
    findings: list[dict[str, Any]], schema_config: dict[str, Any]
) -> None:
