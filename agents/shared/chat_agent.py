...
def _build_tools_section(
    *,
    ...
    parts = [Template(_SECTION_HEADER).safe_substitute(
    ...
    if has_findings and participating_agents:
        ...
        parts.append(Template(_PARTICIPATING_AGENTS_SECTION).safe_substitute(
        ...
        if GUARDRAIL_EVENTS_TABLE:
            try:
                ...
                logger.warning(
                    ...
                    AGENT_TYPE, project_id_log, exc_info=True,
                )
    ...
    if agent_result and hasattr(agent_result, "metrics") and agent_result.metrics:
        try:
            ...
            yield {
                ...
                "tool_calls": sum(
                    tm.call_count
                    for tm in (agent_result.metrics.tool_metrics or {}).values()
                ),
