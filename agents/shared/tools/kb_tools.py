...
- ``lookup_standard`` — queries the Standards KB for compliance
  reference material, filtered by ``standard_id`` and ``section``
  metadata.
...
@handle_tool_errors
def search_document(
    query: str, section_hint: str = "", tool_context: ToolContext = None,
) -> str:
    ...
    if not document_kb_id:
        raise ValueError(
            "search_document missing required invocation_state: ['document_kb_id']"
        )
    if not project_id:
        raise ValueError(
            "search_document missing required invocation_state: ['project_id']"
        )
...
def lookup_standard(
    ...
    Use to cite accurate, current text from standards rather than
    relying on training data. Returns the relevant section text.

    Args:
        standard: Standard identifier (e.g., "owasp-top-10", "gdpr",
            "iso-31000", "dora", "cwe").
        section: Specific section or article (e.g., "Art.32", "A01:2021",
            "CWE-287"). Optional.
        query: Free-text search within the standard if section is unknown.
            At least one of section or query must be provided.
    """
    standards_kb_id = tool_context.invocation_state.get("standards_kb_id")

    if not standards_kb_id:
        raise ValueError(
            "lookup_standard missing required invocation_state: ['standards_kb_id']"
        )

    if not section and not query:
        return (
            "Please provide either a section identifier (e.g., 'Art.32', "
            "'A01:2021') or a free-text query to search within the standard."
        )

    # Build metadata filter — always filter by standard_id, optionally
    # also by section if provided.
    filters = [
        {"equals": {"key": "standard_id", "value": standard}},
    ]
    if section:
    ...
    if len(filters) == 1:
        ...
        metadata_filter = {"andAll": filters}

    # Use section as query if no free-text query provided — the section
    # identifier itself is a reasonable semantic search term.
    search_query = query or section or standard
    ...
    if not results:
        ...
        return (
            f"Standard '{standard}' not found in the corpus. "
            f"Currently indexed standards: {available_str}. "
            f"Cite from your training knowledge and note the citation "
            f"is unverified."
        )
    return _format_kb_results(results)
