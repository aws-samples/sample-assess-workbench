...
def search_findings(query: str, tool_context: ToolContext = None) -> str:
    ...
    if not all([project_id, memory_id, agent_type]):
        ...
        raise ValueError(
            f"search_findings missing required invocation_state: {missing}"
        )
    ...
    return "\n\n".join(
        r.get("content", {}).get("text", "") for r in records
    )
...
def get_prior_findings(
    agent_type: str, query: str = "", tool_context: ToolContext = None,
) -> str:
    ...
    if not all([project_id, memory_id]):
        ...
        raise ValueError(
            f"get_prior_findings missing required invocation_state: {missing}"
        )
