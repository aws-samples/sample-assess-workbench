...
# Trigger a data-source ingestion job and block until COMPLETE. Fails loud
# (exit 1) on FAILED or timeout. Reads KB_ID/DS_ID/AWS_REGION from the caller.
_ingest_and_wait() {
  local job status elapsed=0
  ...
  while [ "${elapsed}" -lt 420 ]; do
    ...
    if [ "${status}" = "COMPLETE" ]; then echo "  ✓ Sync complete"; return 0; fi
  ...
  echo "  ❌ Timeout waiting for ingestion job ${job}"
  exit 1
}
...
# stays valid. Fail loud if the recreated ARN does not match what the KB
# references. Repopulation is handled by the tracking reset + re-sync below.
if [ "$REINDEX" = true ]; then
  ...
  # The recreated index is empty, but Bedrock still tracks the prior docs as
  # ingested and diffs by content hash — so an incremental sync of unchanged
  # objects re-embeds nothing and leaves the index empty. Reset that tracking:
  # purge the objects and run a delete-pass ingestion (records them deleted),
  # so the re-upload + sync below lands every doc as "new".
  echo "  Purging corpus objects to reset ingestion tracking..."
  aws s3 rm "s3://${BUCKET_NAME}/" --recursive --region "${AWS_REGION}"
  echo "  Delete-pass ingestion (clears tracking for the emptied index)..."
  _ingest_and_wait
fi
...
# ── Verify the index is actually queryable ───────────────────────
# A COMPLETE ingestion does not guarantee a populated index: an incremental
# sync into a recreated/empty index re-embeds nothing yet still reports
# COMPLETE, leaving the KB queryable-but-empty (retrieve returns nothing) and
# every compliance agent silently degraded to unverified training knowledge.
# Assert a broad query returns at least one chunk. Retry briefly for index
# consistency after ingestion, then fail loud.
echo ""
echo "Verifying index is queryable..."
VERIFY_ATTEMPTS=0
VERIFY_MAX=6
RESULT_COUNT=0
while [ $VERIFY_ATTEMPTS -lt $VERIFY_MAX ]; do
  RESULT_COUNT=$(aws bedrock-agent-runtime retrieve \
    --knowledge-base-id "${KB_ID}" \
    --retrieval-query '{"text":"compliance standard requirement"}' \
  ...
  if [ -n "${RESULT_COUNT}" ] && [ "${RESULT_COUNT}" != "0" ]; then break; fi
  sleep 10
  VERIFY_ATTEMPTS=$((VERIFY_ATTEMPTS + 1))
done
if [ -z "${RESULT_COUNT}" ] || [ "${RESULT_COUNT}" = "0" ]; then
  echo "  ❌ Standards KB is EMPTY after sync — retrieve returned no results."
  echo "     The vector index has no chunks despite a COMPLETE ingestion job."
  echo "     Re-run with --reindex to force a full re-embed."
  exit 1
fi
echo "  ✓ Index queryable (retrieve returned ${RESULT_COUNT} result(s))"
