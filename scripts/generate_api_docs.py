...
def load_spec(path: Path) -> dict:
    ...
    text = re.sub(
        r"\$\{cognito_issuer_url\}", "https://cognito-idp.<region>.amazonaws.com/<pool_id>", text
    )
...
def _format_parameters(spec: dict, params: list) -> list[str]:
    ...
    for p in params:
        ...
        lines.append(
            f"- `{pname}` ({pin}) {preq} — {pdesc}" if pdesc else f"- `{pname}` ({pin}) {preq}"
        )
    lines.append("")
    return lines
