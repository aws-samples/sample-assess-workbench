...
Source type taxonomy (authority levels):
    ...
    - industry_framework:     OWASP, NIST CSF, ISO 31000, AWS Well-Architected
...
License types:
    ...
    - restricted:    Copyrighted (ISO, PCI DSS) — paraphrase only
...
LicenseType = Literal["cc-by-3.0-au", "open-source", "restricted"]
...
MANIFEST: list[ManifestEntry] = [
    ...
    ManifestEntry(
        standard_id="pci-dss-v4",
        display_name="PCI DSS v4.0",
        source_type="industry_framework",
        license="restricted",
        description=(
            "Paraphrase — copyrighted by PCI SSC. Focus on: encryption requirements, "
            "access control, network segmentation, logging and monitoring, vulnerability "
            "management. Summarise key requirements per section without verbatim reproduction."
        ),
        source=None,  # Held: 397 pages (~204K tokens) exceeds single-pass. Needs page ranges.
        related=[],
        effective_date="March 2022 (mandatory March 2025)",
        enforcement="Mandatory for entities processing payment card data",
    ),
    ManifestEntry(
    ...
    ManifestEntry(
        standard_id="iso-31000",
        display_name="ISO 31000:2018 — Risk Management",
        source_type="industry_framework",
        license="restricted",
        description=(
            "Paraphrase — copyrighted by ISO. Focus on: risk assessment process "
            "(identification, analysis, evaluation), treatment options with examples, "
            "monitoring and review guidance, integration with organisational processes. "
            "Do not reproduce verbatim."
        ),
        source="standards/iso-31000/1580786256_867__AS_2BISO_2B31000-2018_2BRisk_2Bmanagement_2B-_2BGuidelines_2B2001-c030.pdf",
        related=[],
        effective_date="2018",
        enforcement="Advisory — voluntary international standard",
    ),
]
    ...
    for part in pages_str.split(","):
        ...
        if "-" in part:
            ...
            if start < 1 or end > total_pages or start > end:
                raise ValueError(
                    f"Invalid page range '{part}': must be 1-{total_pages}"
                )
            ...
            if page < 1 or page > total_pages:
                raise ValueError(
                    f"Invalid page number '{part}': must be 1-{total_pages}"
                )
            indices.add(page - 1)
...
def _extract_pdf_text(pdf_path: Path, pages: str | None = None) -> str:
    ...
    try:
        ...
        raise ImportError(
            ...
            "Install it: uv pip install PyMuPDF"
        ) from exc
    ...
    if pages:
        ...
        logger.info(
            "Extracting %d of %d pages (ranges: %s)", len(page_indices), total_pages, pages
        )
    else:
        for page in doc:
...
def _fetch_url_text(url: str) -> str:
    ...
    Uses a simple approach: fetch HTML, strip tags, collapse whitespace.
    ...
    # Strip HTML tags (simple regex — sufficient for well-structured pages)
...
LICENSE_INSTRUCTIONS: dict[LicenseType, str] = {
    ...
    "restricted": (
        "This content is copyrighted. You MUST paraphrase all content — do not "
        "reproduce verbatim text. Summarise requirements and guidance in your own "
        "words while preserving the substance and specificity. Note in the output "
        "that the content is paraphrased."
    ),
}
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            if error_code == "ThrottlingException" and attempt < MAX_RETRIES:
                ...
                logger.warning("Throttled, retrying in %ds (attempt %d/%d)", wait, attempt, MAX_RETRIES)
                time.sleep(wait)
                continue
            raise RuntimeError(
...
def list_manifest() -> None:
    ...
    generated = sum(
        1
        for e in MANIFEST
        if (STANDARDS_DIR / e.standard_id / f"{e.standard_id}.md").exists()
    )
...
def main() -> int:
    ...
    if args.all:
        ...
        if not args.force:
            ...
            entries_with_source = [
                e for e in entries_with_source
                if not (STANDARDS_DIR / e.standard_id / f"{e.standard_id}.md").exists()
            ]
            skipped = before - len(entries_with_source)
            if skipped:
                print(f"Skipping {skipped} already-generated standard(s). Use --force to regenerate.\n")
            if not entries_with_source:
                print("All standards with sources are already generated. Use --force to regenerate.")
                return 0
